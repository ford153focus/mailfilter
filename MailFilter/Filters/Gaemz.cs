...
internal class Gaemz
{
    public static void Filter(WrappedMessage wMsg)
    {
        switch (wMsg.Host)
        {
            ...
            case "wasd.tv":
                ...
                return;
            case "white.market":
                wMsg.Move("gaemz // white.market", new List<string> { "gaemz", "white.market" });
                return;
        }
    ...
    public static void Steam(WrappedMessage wMsg)
    {
        ...
        if (Regex.Match(wMsg.Message.Subject, @"(on|from) your Steam wishlist (is|are) (now )?on sale!$").Success ||
            Regex.Match(wMsg.Message.Subject, @"из вашего списка желаемого (в Steam\s+)?прода([еёю])тся со скидкой!$").Success)
        {
